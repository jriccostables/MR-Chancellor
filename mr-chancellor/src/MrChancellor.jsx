...
function useVoice() {
  ...
  const speak = useCallback((text) => {
    ...
    const cleanText = text.replace(/\*\*/g, '').replace(/##/g, '').replace(/\*/g, '').replace(/(\d+)\/(\d+)/g, '$1 to $2').replace(/(\d+)-(\d+)/g, '$1 to $2').replace(/mares/gi, 'mairs').replace(/F&M/g, 'fillies and mairs');
    ...
    <div style={{minHeight:"100vh",background:"#1a1410",color:"#e8dcc8",fontFamily:"Georgia, serif",padding:"20px"}}>
      <div style={{maxWidth:"800px",margin:"0 auto"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"20px"}}>
          ...
          <button onClick={()=>{setEnabled(!enabled);stop();}} style={{background:enabled?"#c9a84c":"#555",border:"none",borderRadius:"8px",padding:"8px 16px",cursor:"pointer",color:"#1a1410",fontWeight:"bold"}}>{enabled?"🔊 Voice On":"🔇 Voice Off"}</button>
        </div>
        {messages.length===0?(<div style={{textAlign:"center",padding:"40px 20px"}}><img src={IMG} style={{width:"180px",borderRadius:"12px",marginBottom:"20px",boxShadow:isSpeaking?"0 0 30px rgba(201,168,76,0.8)":"0 8px 30px rgba(201,168,76,0.3)",transition:"box-shadow 0.3s"}} alt="Mr. Chancellor"/><p style={{color:"#8a7a5a",marginBottom:"30px",fontStyle:"italic"}}>Distinguished Professor of Equine Arts & Racing Sciences</p>{isSpeaking&&<p style={{color:"#c9a84c",marginBottom:"20px"}}>🔊 Speaking...</p>}<div style={{display:"grid",gap:"10px",gridTemplateColumns:"repeat(auto-fit, minmax(250px, 1fr))"}}>{["Analyze this race card for me","How do I rate a horse's mile?","My horse is off - help me spot what's wrong","What's the two-hole strategy at Yonkers?"].map(q=>(<button key={q} onClick={()=>send(q)} style={{background:"#2a2015",border:"1px solid #c9a84c",borderRadius:"8px",padding:"12px",color:"#c9a84c",cursor:"pointer",textAlign:"left",fontSize:"14px"}}>{q}</button>))}</div></div>):(<div>{messages.map((m,i)=>(<div key={i} style={{marginBottom:"20px"}}><div style={{display:"flex",justifyContent:m.role==="user"?"flex-end":"flex-start"}}><div style={{maxWidth:"75%",background:m.role==="user"?"#2c4a1e":"#2a2015",border:"1px solid "+(m.role==="user"?"#4a8a32":"#c9a84c"),borderRadius:"12px",padding:"14px",color:"#e8dcc8",lineHeight:"1.6"}}>{m.content}</div></div>{m.role==="assistant"&&(<div style={{textAlign:"left",marginTop:"5px"}}><button onClick={()=>speak(m.content)} style={{background:"none",border:"none",color:"#c9a84c",cursor:"pointer",fontSize:"13px",fontStyle:"italic"}}>🔈 Hear this again</button></div>)}</div>))}{loading&&<div style={{color:"#c9a84c",fontStyle:"italic"}}>{isSpeaking?"🔊 Speaking...":"Searching racing data..."}</div>}<div ref={endRef}/></div>)}<div style={{position:"sticky",bottom:0,background:"#1a1410",paddingTop:"20px"}}><div style={{display:"flex",gap:"10px",background:"#2a2015",border:"1px solid #c9a84c",borderRadius:"12px",padding:"10px"}}><input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>{if(e.key==="Enter")send();}} placeholder="Ask about races, entries, results, or horse care..." style={{flex:1,background:"transparent",border:"none",color:"#e8dcc8",fontSize:"15px",outline:"none",fontStyle:"italic"}}/><button onClick={()=>send()} disabled={loading} style={{background:"#c9a84c",border:"none",borderRadius:"8px",padding:"8px 16px",cursor:"pointer",fontWeight:"bold"}}>{loading?"...":"Send"}</button></div></div></div></div>
  );
}
